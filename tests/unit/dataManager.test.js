...
describe("DataManager", () => {
  ...
  describe("saveData", () => {
    ...
    it("calls onError when ZIP generation fails", async () => {
      mockZipGenerateAsync.mockRejectedValue(new Error("boom"));
      mockCharacter.images = ["data:image/png;base64,img"];
    ...
    it("uses window.showCustomAlert when no callback provided", async () => {
      mockZipGenerateAsync.mockRejectedValue(new Error("oops"));
      mockCharacter.images = ["data:image/png;base64,img"];
      window.showCustomAlert = jest.fn();
      await dm.saveData(
        mockCharacter,
        mockSkills,
        mockSpecialSkills,
