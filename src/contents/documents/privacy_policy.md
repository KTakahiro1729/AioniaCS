...
- **Google Driveへのアクセス権 (`drive`):**
  キャラクターシートの保存先フォルダを参照・作成し、キャラクターデータの保存・読み込み・共有を行うために使用します。本ツールは、キャラクターシートの管理に必要なフォルダおよびファイルのみを操作し、ユーザーの他の個人的なファイル（写真、文書など）を変更・削除することはありません。
