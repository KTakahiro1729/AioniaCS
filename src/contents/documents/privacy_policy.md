...
本ツールの開発者が管理するサーバー等に、ユーザーのキャラクターシートデータが保存されることはありません。

### 認証セッションデータ

Google Drive連携機能を利用する際、認証セッションの管理のために以下の情報が本ツールのサーバー（Cloudflare D1データベース）に保存されます。
...
これらのデータは認証セッションの維持のみに使用され、セッションの有効期限（最大30日間）が切れた場合、またはユーザーがログアウトした場合に削除されます。
...
- **Cloudflare:** 本ツールのホスティング及び認証セッション管理にCloudflare社のサービス（Cloudflare Pages、D1データベース）を利用しています。認証セッションデータはCloudflare社のインフラストラクチャ上に保存されます。
