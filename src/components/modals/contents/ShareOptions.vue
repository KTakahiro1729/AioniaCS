<template>
    <div class="share-options">
        ...
        <section class="share-options__section">
            ...
            <p v-if="showTruncateWarning" class="share-options__warning">
                内容が一部省略される可能性があります
            </p>
            <div class="share-options__row">
                <label class="share-options__option">
        ...
        <button
            class="button-base share-options__signin"
            v-if="needSignin"
            @click="handleSignin"
        >
            Google Drive にサインイン
...
const needSignin = computed(
    () => (type.value === "dynamic" || includeFull.value) && !props.signedIn
);
const showTruncateWarning = computed(
    () => props.longData && !includeFull.value
);
