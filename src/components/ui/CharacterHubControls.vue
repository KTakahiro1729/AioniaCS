<template>
  <div class="character-hub--controls">
    <template v-if="uiStore.isSignedIn">
      ...
      <button class="button-base character-hub-button character-hub--check" @click="$emit('diagnose')">破損データ診断</button>
    </template>
    <template v-else>
...
const emit = defineEmits(['sign-in', 'sign-out', 'refresh', 'new', 'diagnose']);
