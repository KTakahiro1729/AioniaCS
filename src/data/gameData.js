...
#### 冒険の記録
- **ログイン**: 左上の記録アイコンから「冒険の記録」を開き、アカウントにログインできます。
- **記録と管理**: ログインすると、冒険の記録でクラウド上のキャラクターを一覧表示し、「記録する」「記録を更新する」「読み込む」「削除」などの操作が行えます。

#### 共有機能
- **共有リンク作成**: フッターの「共有」ボタンから、他の人とキャラクターデータを共有するためのリンクを作成できます。
- **スナップショット**: 作成した時点のキャラクターデータのコピーを共有します。
- **ライブリンク**: クラウドストレージに保存されたデータを常に参照するリンクを共有します。データを更新すると、共有相手が見る内容も更新されます。(要クラウド同期)
- **オプション**: 共有リンクにはパスワードや有効期限を設定できます。「全文・画像を含める」オプションもクラウドストレージを利用します。

#### トラブルシューティング
- **クラウドにサインインできない**: ポップアップブロック機能が有効な場合、認証画面が表示されないことがあります。ブラウザの設定で、このサイトのポップアップを許可してください。
